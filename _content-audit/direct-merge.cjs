...
const merged = rawArchive.map((item) => {
  const overrideKey = Object.keys(perfectOverrides).find(
    (key) => item.slug && item.slug.includes(key),
  );
  if (overrideKey) {
    return {
  ...
  for (let b of blocks) {
    ...
    const skips = [
      "dulwich sanctuary 3d garden walkthrough",
      "illuminated custom garden pathway",
      "eye-level view",
      "close-up view",
      "top-down plan view",
      "beautiful conifer garden showcasing",
      "classic wooden garden bench",
      "lush south london garden",
      "cosy compact roof terrace",
      "modern metal garden bench",
      "layered garden planting",
    ];
  ...
  return {
    ...
    excerpt:
      cleanBlocks.length > 0
        ? cleanBlocks[0].substring(0, 150).replace(/\n/g, " ") + "..."
        : "Archived writing.",
    status: "archive",
    contentStatus: "complete",
    content: cleanBlocks,
