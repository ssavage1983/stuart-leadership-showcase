...
function injectAdvancedSEO(filePath, headContent) {
  ...
  // Remove the old basic head function entirely if it exists
  const oldHeadRegex = /head:\s*\(\)\s*=>\s*\(\{\s*meta:\s*\[[\s\S]*?\]\s*\}\),/g;
  content = content.replace(oldHeadRegex, "");

  // Inject the new advanced head function
